...
// 1. 년도 뒷자리 2 숫자를 12로 나눈 몫 : 12년 마다 하루씩 증가
const numQuotient = Math.floor(parseInt(strYearEnd, 10)/12);
// 2. 년도 뒷자리 2 숫자를 12로 나눈 나머지 : 매년 하루씩 증가
const numRemainder = parseInt(strYearEnd, 10)%12;
// 3. 위 나머지를 4로 나눈 값 : 4년마다 유년이 있음
const numQuotient4Remainder = Math.floor(numRemainder/4);
// 4. 년도 뒤 2자리를 제외한 앞자리의 엥커데이 (400년 마다 순환됨)
